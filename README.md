...
- 操作系统：Linux
- 编译器：g++ 12.0.0 或更高版本（需要 C++20 支持）
- Python 3.6+

请使用 git 进行代码管理，所有提交通过 GitHub Classroom 完成。
...
2. 没有占位填充（需要重定位的地方直接用 ❓ 标记，而不是填充 0 来占位）

通过这种设计，我们保留了目标文件格式的核心概念（段、符号、重定位），同时又让它变得清晰易读。在你进行任务时，你可以更多地参考输出的文件（而不是查看`FLEObject`的内存值！）来理解目标文件的结构。
...
这些都与符号（symbol）有关。符号就像程序中的"名字"，代表了函数、变量等。但编译器看到的"名字"和我们写代码时用的并不完全一样。
...
你的第一个任务，就是写一个工具来查看这些符号。这个工具叫 `nm`（name list），它会把符号表以人类可读的方式展示出来。对于上面的代码，它应该输出：
...
现在你已经能看到程序中的符号了。接下来，我们要学习如何把这些符号连接在一起...

## 任务二：实现基础链接器

你有没有想过，当你写了多个源文件时，它们是如何变成一个程序的？

链接器的工作就像是解决一个拼图游戏：每个目标文件都包含了程序的一部分，而链接器需要：
1. 找到每块"拼图"（段）应该放在哪里
2. 解决符号之间的引用（"这个变量在那个文件里"）
3. 调整代码中的地址（因为代码和数据的位置都变了）

为了让你更容易理解这个过程，我们先采用最简单的方案：把所有东西都放在一个叫 `.load` 的段里。就像是把所有的拼图先放在一个大盒子里。
...
提示：
1. 注意 `call` 指令的长度（5 字节）
2. 记得考虑重定位项的 addend（通常是 -4）
3. 用调试打印来验证你的计算
...
在任务三中，我们假设每个符号只在一个地方定义。但实际编程中，经常会遇到同名符号。比如：
...
- 强符号（GLOBAL）：必须唯一，不能重复定义
- 弱符号（WEAK）：可以被强符号覆盖，多个弱符号取第一个

让我们通过一个实际的例子来理解。假设你在写一个库：

```c
// mylib.c
__attribute__((weak))        // 标记为弱符号
void init_logger() {         // 默认的日志初始化函数
    // 使用默认配置
}

void start_logging() {       // 库的主要函数
    init_logger();          // 调用初始化函数
    // 开始记录日志...
}
```
...
使用这个库的程序可以：
1. 直接使用默认的 `init_logger`
2. 提供自己的实现来覆盖它：
   ```c
   // myapp.c
   void init_logger() {     // 强符号，会覆盖弱符号
       // 使用自定义配置
   }
   ```
...
编译器会把这个赋值变成一个新的重定位类型（R_X86_64_64）：
```json
{
    "type": ".obj",
    ".data": [
...
void modify_data(uint8_t* data) {
    data[0] = 42;            // 如果代码段可写，攻击者可以修改程序！
}

void execute_data() {
    uint8_t code[] = {
        0x55, 0x48, 0x89, 0xe5, 0xc3  // 一些机器指令
    };
    ((void(*)())code)();     // 如果数据段可执行，这就是个安全漏洞！
}

void write_code() {
    void* ptr = (void*)write_code;
    *(uint8_t*)ptr = 0x90;  // 如果代码段可写，程序可以被篡改！
}
```

为了防止这些危险操作，现代系统采用分段机制：
1. 代码段（.text）：只读且可执行
2. 只读数据段（.rodata）：只读
3. 数据段（.data）：可读写
